...
import { Boxes, ChevronLeft, ChevronRight, ListChecks, Pencil, Plus, Printer, QrCode, Search, Tag, X } from 'lucide-react'
...
import { EquipmentLabelsModal } from './EquipmentLabelsModal'
...
export function EquipmentPage() {
  ...
  const [labelItems, setLabelItems] = useState<EquipmentItem[]>([])
  ...
  const filtered = allFiltered.slice((currentPage - 1) * pageSize, currentPage * pageSize)
  const paginated = filtered
  const allPageSelected = paginated.length > 0 && paginated.every((item) => selectedIds.has(item.id))
  ...
  function toggleAllPage() { setSelectedIds((current) => { const next = new Set(current); if (allPageSelected) paginated.forEach((item) => next.delete(item.id)); else paginated.forEach((item) => next.add(item.id)); return next }) }
  ...
  return <>
    <div className="title-row clients-title"><div><h1>Equipamentos</h1><p>Itens serializados que saem, voltam e ocupam a agenda.</p></div><div className="title-actions"><button className="secondary" disabled={paginated.length === 0} onClick={toggleAllPage}><ListChecks size={17} /> {allPageSelected ? 'Desmarcar página' : 'Marcar página'}</button><button className="secondary" disabled={selectedItems.length === 0} onClick={() => setLabelItems(selectedItems)}><Printer size={17} /> Imprimir selecionados {selectedItems.length ? `(${selectedItems.length})` : ''}</button><button className="primary" onClick={openNew}><Plus size={17} /> Novo item</button></div></div>
    ...
    {loading ? <p className="empty-state">Carregando equipamentos…</p> : filtered.length === 0 ? <div className="clients-empty"><Boxes /><strong>Nenhum equipamento encontrado</strong><p>Crie uma categoria e cadastre a primeira unidade.</p><button className="primary" onClick={openNew}><Plus size={16} /> Cadastrar item</button></div> : <div className="table-wrap"><table className="clients-table equipment-table"><thead><tr><th><span className="sr-only">Selecionar</span></th><th>Código</th><th>Equipamento</th><th>Empresa</th><th>Categoria</th><th>Marca / modelo</th><th>Local</th><th>Condição</th><th aria-label="Ações" /></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td><input type="checkbox" aria-label={`Selecionar ${item.internal_code}`} checked={selectedIds.has(item.id)} onChange={() => toggleSelection(item.id)} /></td><td><strong>{item.internal_code}</strong></td><td>{item.description}</td><td>{item.organization_name}</td><td>{item.category_name}</td><td>{[item.brand, item.model].filter(Boolean).join(' · ') || '—'}</td><td>{item.storage_location || '—'}</td><td><span className={`condition condition--${item.condition.toLowerCase()}`}>{conditions.find(([value]) => value === item.condition)?.[1]}</span></td><td><div className="row-actions"><button className="icon-action" title="Visualizar etiqueta" aria-label={`Visualizar etiqueta de ${item.internal_code}`} onClick={() => setLabelItems([item])}><QrCode size={15} /></button><button className="icon-action" aria-label={`Editar ${item.internal_code}`} onClick={() => openEdit(item)}><Pencil size={15} /></button></div></td></tr>)}</tbody></table></div>}</section>
    {!loading && allFiltered.length > 0 ? <nav className="table-pagination" aria-label="Paginação de equipamentos"><span>{(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, allFiltered.length)} de {allFiltered.length}</span><label>Por página<select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1) }}><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select></label><button aria-label="Página anterior" disabled={currentPage === 1} onClick={() => setPage((value) => Math.max(1, value - 1))}><ChevronLeft /></button><strong>Página {currentPage} de {pageCount}</strong><button aria-label="Próxima página" disabled={currentPage === pageCount} onClick={() => setPage((value) => Math.min(pageCount, value + 1))}><ChevronRight /></button></nav> : null}
    {labelItems.length > 0 ? <EquipmentLabelsModal items={labelItems} onClose={() => setLabelItems([])} /> : null}
